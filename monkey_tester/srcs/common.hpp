...
#if defined(TEST_MAP) || defined(TEST_SET) || defined(TEST_MULTIMAP) || defined (TEST_MULTISET) || defined(TEST_UNORDERED_MAP) || defined(TEST_UNORDERED_SET) || defined(TEST_UNORDERED_MULTIMAP) || defined (TEST_UNORDERED_MULTISET)
# ifdef TEST_FT
#  include "utility.hpp"
# else
#  include <utility>
...
#if defined(TEST_UNORDERED_MAP) || defined(TEST_UNORDERED_SET) || defined(TEST_UNORDERED_MULTIMAP) || defined (TEST_UNORDERED_MULTISET)
#define UNSTABLE
#endif
...
//			return NAMESPACE::pair<L, R>(*this, *this); since the ordering of equivalent keys in multi-associative containers is undefined we give the same mapped value to all elements
	...
	BidirIt internal_prev( BidirIt it, typename std::iterator_traits<BidirIt>::difference_type n, std::bidirectional_iterator_tag)
	{
		for (;n > 0;--n)
			--it;
		for (;n < 0;++n)
	...
	template< class BidirIt >
	BidirIt internal_prev( BidirIt it, typename std::iterator_traits<BidirIt>::difference_type n, std::input_iterator_tag)
	{
		assert (n <= 0);
		for (;n < 0;++n)
			++it;
	...
	{
		return monkey::internal_prev(it, n, typename std::iterator_traits<BidirIt>::iterator_category());
	}

	// reimplementation of next since we are in c++98
	template< class BidirIt >
	BidirIt next( BidirIt it, typename std::iterator_traits<BidirIt>::difference_type n = 1 )
	{
		return monkey::prev(it, -n);
	}
	...
	void print(const C &c) {
		...
		std::set<typename C::value_type> s(c.begin(), c.end());
			for (typename std::set<typename C::value_type>::const_iterator it = s.begin(); it != s.end(); it++)
	...
	void print(const NAMESPACE::stack<T, C> &c) {
		std::cout << "size: " << c.size() << std::endl;
		std::cout << "content:" << std::endl;
		NAMESPACE::stack<T, C> copy(c);
		for (;!copy.empty(); copy.pop())
			std::cout << "-> [" << copy.top() << "]" << std::endl;
	}
#endif
